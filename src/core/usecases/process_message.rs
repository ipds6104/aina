                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        ...
                        if is_sender_admin && !is_target_group_or_status {
                            ...
                            if !is_quota {
                                let _ = self.whatsapp.send_text_with_session(&msg.chat_jid, &admin_alert, quote_id, msg.session_role).await;
                            }
                            ...
                            if msg.chat_type == ChatType::DirectMessage && !is_target_group_or_status && !is_quota {
                                ...
                                let _ = self.whatsapp.send_text_with_session(&msg.chat_jid, friendly_apology, quote_id, msg.session_role).await;
                            }
                        }
...
mod tests {
    ...
    async fn test_admin_dm_execution_failure_receives_alert_directly_in_dm() {
        ...
        let admin_alert = msgs.iter().find(|(target, _)| target == "6289625345646@s.whatsapp.net");
        assert!(admin_alert.is_some(), "Admin must receive the error notification in DM");
        let (_, alert_text) = admin_alert.unwrap();
        assert!(alert_text.contains("🚨 *Laporan Kegagalan Pemrosesan Pesan Aina*"));
